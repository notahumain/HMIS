...
function bindCommonUI(loader){
  ...
  $('#refreshBtn')?.addEventListener('click', async()=>{ await loader(); toast('Refreshed'); });
  $('#logout')?.addEventListener('click', e=>{e.preventDefault(); localStorage.removeItem('token'); location.href='index.html';});
}
function toast(msg){
...
$('#savePatient')?.addEventListener('click',async e=>{
  ...
  const res=await fetch(API.patients,{method:'POST',headers:{'Content-Type':'application/json',...headers},body:JSON.stringify({name,phone,age,last_visit})});
...
$('#saveAppt')?.addEventListener('click',async e=>{
  ...
  const res=await fetch(API.appointments,{method:'POST',headers:{'Content-Type':'application/json',...headers},body:JSON.stringify({patient_id,doctor_name,scheduled_at,status})});
  const data=await res.json().catch(()=>({}));
  if(!res.ok){alert(data.message||'Failed');return;}
...
$('#saveBill')?.addEventListener('click',async e=>{
  ...
  const res=await fetch(API.billing,{method:'POST',headers:{'Content-Type':'application/json',...headers},body:JSON.stringify({patient_id,items})});
  const data=await res.json().catch(()=>({}));
  if(!res.ok){alert(data.message||'Failed');return;}
